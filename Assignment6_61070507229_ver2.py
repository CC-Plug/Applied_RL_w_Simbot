    ...
    def update(self):
        ...
        if (self.step % 1000 == 0) and (self.step > 0):
            with open('learning_score.csv', 'a', newline='') as csvOpen:
                ...
                c.writerow([self.step/1000, self.eat_count, self.eat_count/self.step , self.collision_count, self.collision_count/self.step])
        self.step += 1
...
if __name__ == '__main__':
    with open('learning_score.csv', 'w+', newline='') as csvOpen:
        c = csv.writer(csvOpen, dialect='excel')
        c.writerow(['iteration', 'eat', 'eat_rate', 'hit', 'hit_rate'])
        c.writerow(
                    [0, 0, 0, 0, 0])

    app = PySimbotApp(robot_cls=RL_Robot, num_robots=1, max_tick=300000, interval= 1/1000.0)
    app.run()